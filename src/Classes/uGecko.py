    ...
    def connect(self, timeout: int = 5) -> None:
        if self.__ip and self.__ip != "":
            if not self.__connected:
                try:
                    if self.__debug_mode: print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} Connecting to {self.__ip}:{self.__port}...")
                    self.s.settimeout(timeout)
                    self.s.connect((str(self.__ip), int(self.__port)))
                    self.__connected = True
                    if self.__debug_mode: print(f"{colorama.Fore.CYAN}[uGecko]:{colorama.Fore.RESET} Connected!")
                except:
